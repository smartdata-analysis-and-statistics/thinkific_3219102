...
df <- data.frame(
  ...
  SE = format(se_indirect, nsmall = 2),
  CI_lower = format(ci[1], nsmall = 2),
  CI_upper = format(ci[2], nsmall = 2)
)
kable(df)
```

This means that, based on indirect evidence through placebo, Semaglutide achieves about `r format(est_indirect, nsmall = 1)`% greater mean weight loss than Liraglutide.
