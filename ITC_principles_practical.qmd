...
Baker2009 <- Baker2009 %>% mutate(
  trtclass = ifelse(treatment %in% c("Fluticasone", "Budesonide"), "ICS",
              ifelse(treatment %in% c("Salmeterol", "Formoterol"), "LABA",
              ifelse(treatment %in% c("Fluticasone+Salmeterol", "Budesonide+Formoterol"), "ICS-LABA",
              ifelse(treatment == "Tiotropium", "LAMA", "Placebo"))))
)
net <- set_agd_arm(data = Baker2009,
                   study = study,
                   trt = treatment,
                   ...
                   r = exac,
                   n = total)
plot(net, weight_edges = TRUE, weight_nodes = TRUE, show_trt_class = TRUE) +
